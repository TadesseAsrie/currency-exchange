...
const CurrencyConverter = () => {
  ...
  // Set default currencies when the list becomes available
  const [fromCurrency, setFromCurrency] = useState('');
  const [toCurrency, setToCurrency] = useState('');
  const [amount, setAmount] = useState(1);
  const [convertedAmount, setConvertedAmount] = useState(0);
  ...
  // Initialize default currencies once data loads
  useEffect(() => {
    if (activeCurrencies.length >= 2) {
      if (!fromCurrency) setFromCurrency(activeCurrencies[0].code);
      if (!toCurrency) setToCurrency(activeCurrencies[1].code);
    }
  }, [activeCurrencies, fromCurrency, toCurrency]);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 dark:text-gray-400">Convert between different currencies instantly</p>
      </div>
            ...
            <div className="grid grid-cols-1 gap-4">
              <div>
                ...
                <Select
                  value={fromCurrency}
                  onChange={(e) => setFromCurrency(e.target.value)}
                  options={activeCurrencies.map(c => ({ value: c.code, label: `${c.code} - ${c.name}` }))}
                />
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                <span className="text-gray-600 dark:text-gray-400">Amount</span>
                <span className="font-semibold">
                  {formatCurrency(amount, fromCurrency || 'USD')}
                </span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                <span className="text-gray-600 dark:text-gray-400">Fees (1%)</span>
                <span className="font-semibold text-red-600">
                  {formatCurrency(fees, toCurrency || 'USD')}
                </span>
              </div>
              <div className="border-t border-gray-200 dark:border-dark-100 pt-3 mt-3">
                <div className="flex justify-between">
                  ...
                  <span className="text-lg font-bold text-primary-600">
                    {formatCurrency(convertedAmount - fees, toCurrency || 'USD')}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3 flex items-start gap-2">
              <FiInfo className="text-blue-500 mt-0.5" />
              <p className="text-sm text-blue-700 dark:text-blue-300">
                Exchange rates are updated in real-time. Fees are calculated at 1% of the converted amount.
              </p>
            </div>
          </div>
        </Card>
